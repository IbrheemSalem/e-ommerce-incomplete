...
@section('content')
    <h1 class="p-relative"><i class="fa-solid fa-right-long mt-1"></i> Edit Subsections</h1>
    <div class="container">
    ...
    <div class="container w-75 mb-5">
        <form class="row g-3" action="{{ route('admin.SubCategory.update',$subcategory->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input name="id" value="{{$subcategory->id}}" type="hidden">
            <div class="text-center">
                <img src="{{asset($subcategory->photo)}}" width="200px" height="200px">
            </div>
            <div class="mb-3">
            ...
            @if(get_languages()->count() > 0)
                    <div class="col-md-6">
                        <label for="inputEmail4" class="form-label">Categories Name : {{__('messages.'.$subcategory->translation_lang)}}</label>
                        <input type="text" class="form-control" value="{{ $subcategory->name }}" name="category[0][name]"  id="inputEmail4">
                        @error("category.0.name")
                        <span class="text-danger">This field is required</span>
                        @enderror
                    </div>
                    <div class="col-md-6 visually-hidden">
                        <label for="inputPassword4" class="form-label">Language abbreviation : {{__('messages.'.$subcategory->translation_lang)}} </label>
                        <input type="text" class="form-control" name="category[0][abbr]" id="inputPassword4" value="{{$subcategory->translation_lang}}">
                        @error("category.0.abbr")
                        <span class="text-danger">This field is required</span>
                        @enderror
                    </div>
                    <div class="col-md-12 d-flex">
                        <label class="form-check-label me-3" for="flexSwitchCheckChecked">active : {{__('messages.'.$subcategory->translation_lang)}} </label>
                        <div class="form-check form-switch ">
                            <input class="form-check-input" type="checkbox" role="switch" value="1" name="category[0][active]" id="flexSwitchCheckChecked" @if($subcategory->active == 1)checked @endif/>
        ...
        <div class="tab mt-4">
            @isset($subcategory->subCategory)
                @foreach ($subcategory->subCategory as $index => $translation )
